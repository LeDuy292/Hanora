      ...
      <section className="max-w-7xl mx-auto w-full px-6 pt-24 pb-12 md:pt-36 md:pb-20 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center z-10 relative">
        ...
        <ScrollReveal className="lg:col-span-5 w-full flex flex-col items-center justify-center relative" animation="scale-in" delay={250}>
          ...
          <div className="relative w-full max-w-[420px] h-[420px] flex items-center justify-center select-none">
            ...
            {/* Floating Left: 2K+ Courses */}
            <div className="absolute left-[-15px] top-[40%] bg-white/95 backdrop-blur-md border border-slate-100/80 px-4 py-3 rounded-2xl shadow-xl flex items-center gap-3 z-20 hover:-translate-y-1 transition-transform duration-200">
              <div className="w-8 h-8 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center">
                <Video className="w-4 h-4" />
              </div>
              <div className="text-left">
                <p className="text-sm font-extrabold text-slate-800 leading-none">2K+</p>
                <p className="text-[9px] font-bold text-slate-500 mt-0.5 whitespace-nowrap">Khóa học Video</p>
              </div>
            </div>

            {/* Floating Right: 5K+ Students with circular progress */}
            <div className="absolute right-[-15px] top-[15%] bg-white/95 backdrop-blur-md border border-slate-100/80 px-4 py-3.5 rounded-2xl shadow-xl flex flex-col items-center gap-2 z-20 hover:-translate-y-1 transition-transform duration-200">
              <div className="relative w-9 h-9 flex items-center justify-center">
                <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                  <circle cx="18" cy="18" r="16" fill="none" stroke="#f1f5f9" strokeWidth="3" />
                  <circle cx="18" cy="18" r="16" fill="none" stroke="#2563eb" strokeWidth="3" strokeDasharray="100" strokeDashoffset="25" />
                </svg>
                <span className="absolute text-[9px] font-bold text-blue-600">75%</span>
              </div>
              <div className="text-center">
                <p className="text-xs font-extrabold text-slate-800 leading-none">5K+</p>
                <p className="text-[9px] font-bold text-slate-500 mt-0.5 whitespace-nowrap">Học viên trực tuyến</p>
              </div>
            </div>

            {/* Floating Bottom: Tutors 250+ */}
            <div className="absolute right-[20px] bottom-[15%] bg-white/95 backdrop-blur-md border border-slate-100/80 px-4 py-2.5 rounded-2xl shadow-xl flex items-center gap-2.5 z-20 hover:-translate-y-1 transition-transform duration-200">
              <div className="w-7 h-7 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center">
                <GraduationCap className="w-4 h-4" />
              </div>
              <div className="text-left">
                <p className="text-xs font-extrabold text-slate-800 leading-none">Giảng viên</p>
                <p className="text-[10px] font-black text-blue-600 mt-0.5">250+</p>
              </div>
            </div>
              ...
              {demoWords.map((token, idx) => {
                ...
                return (
                  <span
                    ...
                    className={`inline-block mx-0.5 cursor-pointer rounded transition-all duration-150 py-1 ${
                      isSelected
